...
// Backfill strategy: if cache makes us run out, relax ONLY the score threshold (keep exclusions)
// so multiple calls in the same day still return ~same amount of items.
const relaxedAll = filterStories(stories, { minPoints: 0 });
const relaxedFresh = filterUnseen(relaxedAll, cache);

const primary = dropShowFromMain(keptFresh);
const fillerPool = dropShowFromMain(relaxedFresh);

const seenIds = new Set(primary.map(s => s.id));
const filler = fillerPool.filter(s => !seenIds.has(s.id));

const main = [...primary, ...filler].slice(0, opts.top);
